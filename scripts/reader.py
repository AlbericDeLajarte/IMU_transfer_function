...
class mpu_reader:
    ...
    def handle_raw_data(self, i):
        ...
        # Get raw data
        if self.mpu != None:
            self.accel_data.x, self.accel_data.y, self.accel_data.z = self.mpu.acceleration
            self.gyro_data.x, self.gyro_data.y, self.gyro_data.z = self.mpu.gyro
...
def reader():
    ...
    rate = rospy.Rate(1/time_period) # 100hz
